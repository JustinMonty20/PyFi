...
# some figures are returned as percentages. Other values are not because the return values are needed to calcuate other functions.
...
def portfolio_return(weights, *stocks):
    ...
    if len(weights) == len(stocks) and sum(weights) == 1:
        ...
        raise RuntimeError(
            'Make sure your weights add to 1 and the lengths of stocks and weights you are passing in are the same.')
...
def portfolio_volatility(weights, secs):
    ...
    if len(weights) == len(secs) and sum(weights) == 1.0:
        ...
        raise error(
            'Make sure your weights add to 1 and the lengths of sec and weights you are passing in are the same.')
